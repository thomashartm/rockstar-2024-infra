locals {
  event_rule_name = "${var.name_prefix}-event-rule"
}

#------------------------------------------------------------------------------
# CLOUDWATCH EVENT ROLE
#------------------------------------------------------------------------------
data "aws_iam_policy_document" "scheduled_task_assume_role_policy" {
  statement {
    effect  = "Allow"
...
data "aws_iam_policy_document" "scheduled_task_cloudwatch_policy" {
  ...
  statement {
    ...
    resources = var.ecs_task_role_arn == null ? [var.ecs_execution_task_role_arn] : [
      var.ecs_execution_task_role_arn, var.ecs_task_role_arn
    ]
  }
}
...
resource "aws_iam_role" "scheduled_task_cw_event_role" {
  ...
  assume_role_policy = data.aws_iam_policy_document.scheduled_task_assume_role_policy.json
}
